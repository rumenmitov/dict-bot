...
client.on('message', (message) => {
    if (message.content[0] === PREFIX) {
        let user_message = message.content.split('/')[1];
        if (user_message === 'help') {
            message.channel.send("Hello! I'm donke from Shrek! My goal is to aid you to uncover the definitions of unknown words! Type **/define** followed by a word!");
        }
...
async function getImage(word) {
    const response = await axios.get(`https://api.unsplash.com/photos/random?query=${word}&orientation=landscape&order_by=relevant`, {
        headers: {
